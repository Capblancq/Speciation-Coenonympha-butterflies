...
def admix(params, ns, pts):
    ## parse params
    N1,N2,N3,T2,T1,m23,m32 ,f = params
...
while i < 1:
	...
	params_admix = np.array([N1,N2,N3,T2,T1,m23,m32,f])

	## search limits
	upper_admix = [100, 100, 100,  10,  2, 40, 40, 0.99999]
	lower_admix = [1e-2, 1e-2, 1e-2, 1e-3, 1e-3, 0, 0, 0.00001]
	...
	os.mkdir("./RESULTS_HS_geneflow_gardetta/" + outputname)
	output_file = open(("./RESULTS_HS_geneflow_gardetta/" + outputname + "/" + outputname + ".txt"), "w")
	line = ("\n" + "HS_geneflow_gardetta" + "\n" + "\n" "Optimization : " + repr("BFGS") + "\n"  "Optimized parameters: " + repr(popt) + "\n" + "Optimized log-likelihood: " + repr(ll_opt) + "\n" + "theta: " + repr(theta) + "\n" + "AIC: " + repr(AIC) + "\n")
	...
	#pylab.savefig(("./RESULTS_HS_geneflow_gardetta/" + outputname + "/" + outputname + "_" + "HS_geneflow_gardetta" + ".png"), dpi=300)
	#pylab.close(fig)
